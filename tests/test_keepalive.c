...
task_t* task_arg(read_one_response_) {
    typedef struct { async_stream_t *s; char *buf; size_t cap; size_t *lenp; } args_t;
    gen_dec_vars(
        args_t a;
        char chunk[1024];
        int n;
    ...
    while (1) {
        gen_var(task) = async_stream_read(gen_var(a).s, sizeof(gen_var(chunk)), gen_var(chunk));
...
task_t* read_one_response(async_stream_t *s, char *buf, size_t cap, size_t *lenp) {
    struct { async_stream_t *s; char *buf; size_t cap; size_t *lenp; } *a = malloc(sizeof(*a));
...
task_t* task(keepalive_client_task) {
    ...
    /* 建单条连接到服务器 */
    gen_var(sock) = async_socket_create(anet_palsock_create(AF_INET, SOCK_STREAM, 0, 1));
    {
        ...
        gen_var(cfut) = async_socket_connect(gen_var(sock), (struct sockaddr*)&sin, sizeof(sin));
        gen_yield(gen_var(cfut));
        if (anet_code_of(future_result(gen_var(cfut))) != 0) { printf("connect failed\n"); exit(1); }
    }
    gen_var(stream) = async_stream_from_socket(gen_var(sock));

    /* --- 请求 1 (keep-alive 默认) --- */
    snprintf(gen_var(req), sizeof(gen_var(req)),
             "GET /one HTTP/1.1\r\nHost: x\r\n\r\n");
    gen_var(task) = async_stream_write_all(gen_var(stream), gen_var(req), strlen(gen_var(req)));
    gen_yield_from_task(gen_var(task));
    ...
    async_stream_destroy(gen_var(stream));
